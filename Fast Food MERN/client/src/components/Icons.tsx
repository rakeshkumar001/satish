...
export default function Icons({ iconName, classname }: { iconName: string, classname?: string }) {
    switch (iconName) {
        ...
        case 'favourite':
            return <StarIcon className={classname ? classname : 'h-5 w-5'} />
        case 'foodType':
            return <ViewfinderCircleIcon className={classname ? classname : 'h-5 w-5'} />
        case 'search':
            return <MagnifyingGlassIcon className={classname ? classname : 'h-5 w-5'} />
        case 'time':
            return <ClockIcon className={classname ? classname : 'h-5 w-5'} />
        case 'heart':
            return <HeartIcon className={classname ? classname : 'h-5 w-5'} />
        case 'note':
            return <ExclamationCircleIcon className={classname ? classname : 'h-5 w-5'} />
        // case 'heart':
        //     return <HeartIcon className={classname ? classname : 'h-5 w-5'} />
        // case 'heart':
        //     return <HeartIcon className={classname ? classname : 'h-5 w-5'} />
